...
public class MR_App_Master{
	...
	public MR_App_Master(Workload_Parameters wp,VM_Parameters vm,int workload_num)throws IOException{
		...
		//wjs.write_2D(Reduce_status, "Reduce Status of job/thread name  "+wp.workload_name[workload_num]+"  (10 columns: reduce task #, amount of data to process, status(not_yet_started(0),running(1)/finished(2)), response time, start time, end time, latency, bin #, VM flavor #,PM #, Rack #)");
	} 	
